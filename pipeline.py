...
def run_single(cfg: DictConfig) -> Dict:
    ...
    # id for saving
    save_id = cfg.data.image_file.split("/")[-1].split(".")[0]# cfg.data.get("save_id", "sample")
    ...
    if cfg.get("save_all_heads", False):
        save_all_heads(attn, meta, attn_root)
        grid_path = os.path.join(attn_root, f"{save_id}_all_heads_overview.png")
...
def run_analyze_visualize(cfg: DictConfig) -> None:
    ...
    # Save all heads if enabled
    if cfg.get("save_all_heads", False):
        save_all_heads(attn, meta, out_dir)
        grid_path = cfg.data.attention_file.replace('.pkl', '_all_heads_overview.png')
        save_all_heads_grid(attn, meta, grid_path, heads_per_row=8)
...
def run_batch(cfg: DictConfig) -> None:
    ...
    for i, entry in enumerate(work):
        ...
        if cfg.get("save_all_heads", False):
            save_all_heads(attn, meta, attn_root)
            if cfg.data.visualize_batch:
                grid_path = os.path.join(attn_root, f"{sid}_all_heads_overview.png")
                save_all_heads_grid(attn, meta, grid_path, heads_per_row=8)
        P = int(meta["patch_size"])  # grid size
        combo = combine_heads(attn, selected[: cfg.logic.top_k], P=P, sigma=cfg.logic.smoothing.sigma)
        mask_grid = binarize_mean_relu(combo)
